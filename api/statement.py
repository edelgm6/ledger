...
class Trend:
    ...
    def _get_month_ranges(self):
        ...
        while current_date <= self.end_date:
            ...
            end_of_month = min(
                end_of_month, self.end_date
            )  # Ensure end of month is not greater than end date
    ...
    def get_balances(self):
        ...
        for range in ranges:
            income_statement = IncomeStatement(
                end_date=range.end, start_date=range.start
            )
...
class Statement:
    ...
    def _get_balance_from_aggregates(aggregates, end_date=None, balance_type=None):
        ...
        for aggregate in aggregates:
            ...
            balance = Account.get_balance_from_debit_and_credit(
                account.type, debits=debits, credits=credits
            )
            account_balance_list.append(
                Balance(account, balance, end_date, balance_type)
            )
    ...
    def get_balances(self):
        if isinstance(self, IncomeStatement):
            ...
            aggregates = JournalEntryItem.objects.filter(
                account__type__in=ACCOUNT_TYPES, journal_entry__date__lte=self.end_date
            )
        ...
        type_objects = [
            Account.Type[type_value.upper()] for type_value in ACCOUNT_TYPES
        ]
        ...
        balances = self._get_balance_from_aggregates(
            aggregates, self.end_date, balance_type
        )
        return balances
...
class CashFlowStatement(Statement):
    ...
    def __init__(self, income_statement, start_balance_sheet, end_balance_sheet):
        ...
        for balances_list in [
            self.cash_from_operations_balances,
            self.cash_from_financing_balances,
            self.cash_from_investing_balances,
        ]:
        ...
        self.summaries = [
            ...
            Metric(
                ...
                self.get_cash_flow(self.cash_from_operations_balances),
            ),
            Metric(
                "Cash Flow From Investing",
                self.get_cash_flow(self.cash_from_investing_balances),
            ),
            Metric(
                "Cash Flow From Financing",
                self.get_cash_flow(self.cash_from_financing_balances),
            ),
        ...
        self.metrics = [
            Metric(
                "Levered post-tax Free Cash Flow",
    ...
    def get_cash_balance(balance_sheet):
        cash = sum(
            [
                summary.value
                for summary in balance_sheet.summaries
                if summary.name == "Cash"
            ]
        )
        return cash
    ...
    def get_levered_after_tax_cash_flow(self):
        return self.net_income_less_gains_and_losses + sum(
            [
                summary.value
                for summary in self.summaries
                if summary.name == "Cash Flow From Financing"
            ]
        )
    ...
    def get_balance_sheet_account_deltas(self):
        accounts = Account.objects.filter(
            type__in=[Account.Type.ASSET, Account.Type.LIABILITY, Account.Type.EQUITY]
        )
        account_deltas = []
        for account in accounts:
            starting_balance = sum(
                [
                    balance.amount
                    for balance in self.start_balance_sheet.balances
            ...
            account_deltas.append(
                Balance(
                    account=account, amount=delta, date=self.end_balance_sheet.end_date
                )
            )
    ...
    def get_cash_from_operations_balances(self):
        ...
        accounts_receivable_accounts = [
            ...
            if balance.account.sub_type == Account.SubType.ACCOUNTS_RECEIVABLE
        ]
        short_term_debt_accounts = [
            balance
            for balance in self.balance_sheet_deltas
            if balance.account.sub_type == Account.SubType.SHORT_TERM_DEBT
        ]
        taxes_payable_accounts = [
            balance
            for balance in self.balance_sheet_deltas
            if balance.account.sub_type == Account.SubType.TAXES_PAYABLE
        ]
    ...
    def get_cash_from_financing_balances(self):
        long_term_debt = [
            balance
            for balance in self.balance_sheet_deltas
            if balance.account.sub_type == Account.SubType.LONG_TERM_DEBT
        ]
    ...
    def get_cash_from_investing_balances(self):
        ...
        for item in journal_entry_items:
            ...
            adjustment = (
                item.amount
                if account.type in [Account.Type.LIABILITY, Account.Type.INCOME]
                else -item.amount
            )
            adjustment *= (
                -1 if item.type == JournalEntryItem.JournalEntryType.CREDIT else 1
            )
        ...
        balances = [
            Balance(account=key, amount=value, date=self.end_balance_sheet.end_date)
    ...
    def get_tax_rate(self):
        taxable_income = sum(
            [
                ...
                if balance.account.sub_type
                in [
                    Account.SubType.SALARY,
                    Account.SubType.DIVIDENDS_AND_INTEREST,
                    Account.SubType.REALIZED_INVESTMENT_GAINS,
                ]
            ]
        )
        taxes = sum(
            [
                balance.amount
                for balance in self.balances
                if balance.account.sub_type == Account.SubType.TAX
            ]
        )
        if taxable_income == 0:
            return None
...
class BalanceSheet(Statement):
    ...
    def get_retained_earnings_values(self):
        income_statement = IncomeStatement(
            end_date=self.end_date, start_date="1970-01-01"
        )
        retained_earnings = income_statement.get_net_income()

        investment_gains_losses = sum(
            [
                balance.amount
                for balance in income_statement.balances
                if balance.account.sub_type
                == Account.SubType.UNREALIZED_INVESTMENT_GAINS
            ]
        )
    ...
    def get_cash_percent_assets(self):
        cash = sum(
            [
                summary.value
                for summary in self.summaries
                if summary.name == Account.SubType.CASH.label
            ]
        )
        assets = sum(
            [
                summary.value
                for summary in self.summaries
                if summary.name == Account.Type.ASSET.label
            ]
        )
        if assets == 0:
            return None
    ...
    def get_debt_to_equity(self):
        liabilities = sum(
            [
                summary.value
                for summary in self.summaries
                if summary.name == Account.Type.LIABILITY.label
            ]
        )
        equity = sum(
            [
                summary.value
                for summary in self.summaries
                if summary.name == Account.Type.EQUITY.label
            ]
        )
        if equity == 0:
            return None
    ...
    def get_liquid_assets(self):
        ...
        brokerage = sum(
            [
                summary.value
                for summary in self.summaries
                if summary.name == Account.SubType.SECURITIES_UNRESTRICTED.label
            ]
        )
